...
const SQUADS = ["MASTER", "ELITE", "SQT1", "SQT2", "SQT3"];
  ...
  return (
    <div className="space-y-6">
      ...
      <Tabs defaultValue="visao-geral" className="w-full">
          ...
          <div className="rounded-xl border border-border bg-card overflow-hidden">
            ...
            <div className="grid grid-cols-[2fr_1.5fr_1fr_1fr_80px] px-6 py-3 border-b border-border">
              {["COLABORADOR", "CARGO", "SQUAD", "STATUS", "AÇÕES"].map((h) => (
                <span key={h} className="text-[11px] font-semibold tracking-widest text-muted-foreground uppercase">
                  {h}
                </span>
              ))}
            </div>
            ...
            {filtered.map((user) => {
              ...
              return (
                <div
                  key={user.id}
                  className="grid grid-cols-[2fr_1.5fr_1fr_1fr_80px] items-center px-6 py-4 border-b border-border last:border-0 hover:bg-muted/30 transition-colors"
                >
                  {/* Colaborador */}
                  <div className="flex items-center gap-3">
        ...
        {/* ── Gestão de Squads ── */}
        <TabsContent value="gestao-squads" className="mt-6"><div /></TabsContent>
